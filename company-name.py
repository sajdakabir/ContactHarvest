...
apollo_email_enricher.py
---------------------------------
Find execs by company *name* or *domain* and reveal emails.

Example:
    python3 apollo_email_enricher.py "Switch Software"
    python3 apollo_email_enricher.py switchsoftware.io
"""
...
def search_people(
    ...
    # Decide whether user passed a domain or a name
    if "." in company:                    # looks like a domain
...
def enrich_person(person_id: str) -> Optional[str]:
    """Reveal a personal email via People Match; return email or None."""
    headers = {"x-api-key": API_KEY, "Content-Type": "application/json"}
...
def fetch_emails(
    ...
    """Return (first, last, email, title, linkedin) tuples."""
    results = []
    for page in range(1, max_pages + 1):
        people = search_people(company, titles, page=page)
        if not people:
            break
...
def pretty_print(company: str, rows: List[Tuple[str, str, str, str, str]]) -> None:
    print(f"\n🔍  Searching for people at: {company}")
    website_hint = f"https://{company}" if "." in company else "(domain unknown)"
    print(f"🌐  Company Website: {website_hint}\n")
...
def main() -> None:
    ...
    contacts = fetch_emails(company, titles=DEFAULT_TITLES)
    pretty_print(company, contacts)
